...
public class UpDownState : BaseSpawnerState
{
    public override void EnterState(SpawnerScript ss)
    {
        ...
        {
            if (Physics.Raycast(ss.spawnSpots[i].transform.position + ss.spawnSpots[i].transform.localScale / 2, Vector3.up, out ss.hit, 1) == false)
            {
                ss.upSpace[i] = true;
            }
            if (Physics.Raycast(ss.spawnSpots[i].transform.position - ss.spawnSpots[i].transform.localScale / 2, Vector3.down, out ss.hit, 1) == false)
            {
                ss.downSpace[i] = true;
            }
        }
